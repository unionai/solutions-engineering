...
async def search_docs(query: str, max_results: int = 3) -> list[dict[str, str]]:
    ...
    corpus = [
        {
            "title": "Tasks",
            "body": "Define a task by decorating an async function with @env.task.",
        },
        {
            "title": "Triggers",
            "body": "Schedule a task by attaching a flyte.Trigger with a flyte.Cron automation.",
        },
        {
            "title": "Secrets",
            "body": "Mount cluster-managed secrets into a task with flyte.Secret(...).",
        },
    ]
    needle = query.lower()
    matches = [
        d for d in corpus if needle in d["body"].lower() or needle in d["title"].lower()
    ]
