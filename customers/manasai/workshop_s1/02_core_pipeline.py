"""
Core Pipeline — Sections 1-5 in one workflow
=============================================
...
- Data & I/O:          typed values between tasks, a dataclass result,
...
# Data I/O: a dataclass gives the pipeline a typed, structured result.
@dataclass
class Summary:
    chunks: int
    lines: int
...
def build_chunk_lines(chunk_id: int, lines: int) -> list[str]:
    ...
    return [f"chunk {chunk_id}, line {i}: hello from flyte {sparkle}" for i in range(lines)]
